...
Quick filter unification (AQ-confirmed): plain typing = case-insensitive substring; leading "/" starts case-sensitive regex ("/" consumed); "/pat/i" suffix makes it case-insensitive regex; remove all toggles (case/whole/fade/regex) + fadeUnmatched mode (filter then ESC to reveal all); persistent small "?" help icon (right end, 0-delay hover tooltip) documenting rules + /^pat$/ for whole name + ESC; invalid /regex tints input red + error tooltip; drop QuickFilterOptions + storage + wiring entirely; full i18n (14 locales) for help/error strings; update tests/README. Shipped UX change → patch bump on behavior commit. Deferred: dedicated preview content (text/media/EXIF), quick-actions bar, multi-select summary, palette recent-use / keybinding ranking boosts.
